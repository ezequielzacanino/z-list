  ...
  return (
    <>
      {[-1, 1].map((side) => (
        <path
          key={side}
          d={`M${32 + side * width * 0.3} 56q${side * 3} 1 ${side * (3 + level('roots') * 1.6)} 2.5`}
          {...line(WOOD.shade, 2)}
        />
      ))}
      ...
      {branches > 0 &&
        [1, -1].slice(0, branches > 1 ? 2 : 1).map((side) => {
          ...
          return (
            <g key={side}>
              <Tube
                d={`M${32 + side * width * 0.4} ${armY}q${side * 5} -1 ${side * 8} ${endY - armY}`}
                color={WOOD}
                width={1.8}
              />
              <Leaf x={32 + side * (width * 0.4 + 8)} y={endY} size={2.4} angle={side * 30} />
            </g>
          )
        })}
      <path
        d={`M${32 - width / 2} 57.5Q${32 - width / 2 + 1} ${top + 6} ${32 - width * 0.35} ${top}H${32 + width * 0.35}Q${32 + width / 2 - 1} ${top + 6} ${32 + width / 2} 57.5z`}
        {...paint(WOOD)}
      />
      <path
        d={`M${32 - width * 0.25} ${top + 4}v4M${32 + width * 0.2} ${57 - 6}v3`}
        {...line(WOOD.shade, 0.8)}
        opacity={0.5}
      />
      {leaves === 1 && (
        ...
        <g>
          ...
          {level('nest') > 1 && (
            <>
              ...
              />
              <path
                d={`M${32 + width * 0.4 + 8.6} ${armY - 10.6}l1.2 .3l-1.2 .4z`}
                fill="#ffd166"
              />
            </>
          )}
      ...
      <Eye x={32 - 2.6} y={faceY} r={1.4} />
      <Eye x={32 + 2.6} y={faceY} r={1.4} />
      <Blush x={32 - 4} y={faceY + 2.4} r={1.2} />
      <Blush x={32 + 4} y={faceY + 2.4} r={1.2} />
      <Smile x={32} y={faceY + 2.4} w={1.4} />
      {level('face') > 0 &&
        spread(4, 28.5, 35.5).map((x) => (
          <circle key={x} cx={x} cy={faceY + 5.2} r={1.3} fill={LEAF.fill} />
        ))}
    </>
  )
}
  ...
  return (
    <>
      ...
      {[-1, 1].map((side) => (
        <path
          key={side}
          d={`M${32 + side * 3} 55.5q${side * 1} 2 ${side * 3} 2.8`}
          {...line(root.shade, 1.8)}
        />
      ))}
      {level('arms') > 0 &&
        [-1, 1].map((side) => {
          ...
          return (
            <Tube
              key={side}
              d={`M${32 + side * width * 0.75} ${centerY}q${side * 4} ${raised ? -3 : 2} ${side * 5} ${raised ? -7 : 5}`}
              color={root}
              width={1.8}
            />
          )
      ...
      <path
        d={`M32 ${top}C${32 + width} ${top + 1} ${32 + width} 55 32 57C${32 - width} 55 ${32 - width} ${top + 1} 32 ${top}z`}
        {...paint(root)}
      />
      <path
        d={`M${32 - width * 0.4} ${centerY + 4}q2 .6 3 0M${32 + width * 0.2} ${centerY + 6}q2 .6 3 0`}
        {...line(root.shade, 0.7)}
        opacity={0.5}
      />
      {level('crown') > 0 &&
      ...
      {mouth > 1 && (
        <path
          d="M44 30v-5l3-1v5M44 30a1.2 1 0 1 1-.1 0M47 29a1.2 1 0 1 1-.1 0"
          {...line(INK, 0.9)}
        />
      )}
