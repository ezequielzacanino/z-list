...
export function ListPage({ userId }: { userId: string }) {
  ...
  function authorName(item: Item) {
    return item.created_by && item.created_by !== userId
  ...
  async function invite(email: string) {
    ...
    if (mailed) {
      setInviteNotice(
        `Le mandamos un mail a ${email} para que cree su cuenta.`,
      );
      await reloadMembers();
    }
    return mailed;
  ...
  return (
    <div className="stack">
      <header className="row">
        ...
        <div className="row actions">
          ...
          <button
            className="toggle"
            onClick={() =>
              updateList({ sort_by_priority: !list.sort_by_priority })
            }
          >
      ...
      {!items.length && (
        <p className="muted">La lista está vacía. Agregá algo arriba.</p>
      )}
      ...
      {done.length > 0 && (
        <section className="history">
          <h2>Historial</h2>
          <ul className="items">
            {done.slice(0, historyLimit).map((item) => (
              <ItemRow
                key={item.id}
                item={item}
                authorName={authorName(item)}
                onToggle={
                  copied.has(item.id) ? undefined : () => toggleItem(item)
                }
                onOpen={() => setOpenItemId(item.id)}
              />
            ))}
          </ul>
          {done.length > historyLimit && (
            <button
              className="ghost history-more"
              onClick={() => setHistoryLimit(historyLimit + HISTORY_PAGE)}
            >
              Ver {Math.min(HISTORY_PAGE, done.length - historyLimit)} más
            </button>
          )}
        </section>
      )}
